...
export default function HistoryPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-playfair">
      ...
      {/* ── Year Timeline Tabs ── */}
      <div 
        className={`border-b border-gray-400 sticky z-40 bg-white transition-[top] duration-300 ease-in-out ${
          showNav ? "top-[70px] md:top-[130px]" : "top-0"
        }`}
      >
        <div
          ref={timelineRef}
        ...
        >
          {HISTORY_DATA.map((item, i) => (
            <button
              ...
              data-index={i}
              onClick={() => goTo(i)}
              className={`flex-shrink-0 px-6 py-5 text-sm tracking-[0.2em] uppercase transition-all duration-200 border-b-2 font-medium ${
                i === activeIndex
      ...
      <section className="max-w-4xl mx-auto px-6 md:px-10 py-20">
        ...
        >
          ...
          {/* Subtitle */}
          {/* <p
            className="text-center text-xs tracking-[0.3em] uppercase text-gray-400 mb-12"
            style={{ fontFamily: "Playfair Display, serif", fontStyle: "italic" }}
          >
            {activeItem.subtitle}
          </p> */}
          ...
          {/* Quote block */}
          <div className="bg-black text-white px-8 py-10 mb-12 text-center">
            <p
